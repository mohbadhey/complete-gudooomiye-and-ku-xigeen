    ...
    [WebMethod]
        ...
        public static object candi11()
        {
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                ...
                // Example SQL query to retrieve count, name, and image URL for cand_id 4, 5, and 6
                SqlCommand cmd = new SqlCommand("SELECT COUNT(*) AS Count, c.name AS Name, c.images AS ImageUrl FROM votes v INNER JOIN candidates c ON v.cand_id = c.cand_id WHERE v.cand_id IN (1, 2, 3) GROUP BY c.name, c.images", con);
        ...
        public static object candi1111()
        {
            ...
            using (SqlConnection con = new SqlConnection(cs))
            {
                ...
                // Example SQL query to retrieve count, name, and image URL for cand_id 4, 5, and 6
                SqlCommand cmd = new SqlCommand("SELECT COUNT(*) AS Count, c.name AS Name, c.images AS ImageUrl FROM votes v INNER JOIN candidates c ON v.cand_id = c.cand_id WHERE v.cand_id IN (4, 5, 6) GROUP BY c.name, c.images", con);
                SqlDataReader reader = cmd.ExecuteReader();
